...
description: Run colleagues under their own @mention name with a per-colleague Slack app.
icon: wand-sparkles
---
...
<Steps>
  ...
  <Step title="Fill in three fields">
    ...
    - `display_information.name` — the app's display name in Slack admin. Use the colleague's name, like `Sofia` or `Acme Sales Colleague`.
...
Each custom Slack app represents one colleague. To run a second branded colleague — `@Max` for customer success, on top of `@Sofia` for sales — repeat all three steps with a fresh manifest, fresh credentials, and a fresh install. You can run as many as you want. They coexist in the same Slack workspace under different `@names`.
