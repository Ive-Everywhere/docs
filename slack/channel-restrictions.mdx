...
description: Pin a colleague to specific Slack channels with allow lists, keep them out of sensitive channels with block lists, or let them answer everywhere.
icon: lock
---
...
The change takes effect immediately. Existing threads continue, but the colleague ignores new mentions in disallowed channels.
...
Private channels work the same as public ones — pick them in the picker, add them to the list. A workspace member still needs to invite the colleague to the channel before it can respond there, regardless of allow-list state.
