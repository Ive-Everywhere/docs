...
You can also connect a Claude OAuth account (the same sign-in Anthropic offers Pro users). That uses your subscription rather than usage-based billing.

**Other provider keys.** OpenAI, Google Gemini, DeepSeek, Z-AI, Moonshot, xAI. Paste each in the corresponding field.

## How keys are stored

Eluu encrypts every key at rest. You can't view a key after saving it — if you need to update one, paste the new one and save again. There's no way to view the old one back.
