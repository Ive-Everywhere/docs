...
Shared connections work for everyone on the team — but each user authenticates with their own account. Alice's Gmail and Bob's Gmail both run through the same shared connection. Alice never sees Bob's emails. See [Personal vs Shared](/integrations/team-vs-personal).
