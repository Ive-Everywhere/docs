...
Once saved, the colleague is reachable from every surface — web app, Slack, browser extension — and remembers everything across sessions.

## How many colleagues you should have

Most teams start with 3-5 colleagues, each focused on a different role. A useful pattern:

- **A general assistant** — the default colleague Eluu created for you on signup.
...
Building more colleagues is cheap. They're just configuration. **Don't pile every responsibility on one colleague.** A colleague that does five unrelated jobs is worse than five colleagues each doing one. Sharper roles produce sharper results — and the workspace memory of a focused colleague compounds faster.
...
Members can still chat with shared colleagues. They just can't change the configuration.
