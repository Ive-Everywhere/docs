...
description: Connect Eluu to Gmail, Slack, Sheets, Salesforce, HubSpot, Notion, GitHub, and more so colleagues can read, write, and act across your stack.
icon: plug
---
...
If you connect a tool first and add colleagues later, you can enable it on a colleague any time. Edit the colleague, go to **Integrations**, and toggle the connection on. Same the other way — turning a colleague's access back on takes one click.
...
Connections occasionally expire or get revoked by the provider. When that happens, the tool shows a `Needs reauth` label. Click into it and re-run the sign-in. Anything that was waiting on the tool resumes once it's reconnected.
