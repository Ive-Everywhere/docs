...
Slash commands are pre-packaged playbooks you trigger from the chat. Type `/` to open the command picker, pick one, then add your prompt.

## How they work

<Steps>
  <Step title="Type a forward slash">
    At the start of the chat input, type `/`. The command picker opens above it.
  </Step>
  ...
  <Step title="Type your message">
    The command becomes a chip in the chat input. Type your actual prompt after it. Press Enter to send.
  </Step>
</Steps>
...
The selected command becomes a dismissible chip before the chat input. To remove it:

- Click the × on the chip.
- Or press Backspace before any text.

## Quick actions

When you open a new chat, the four most-used commands appear as quick-action cards above the chat input. Clicking one is the same as typing `/{name}` — it pre-fills the chip so you can add your prompt.
...
<CardGroup cols={2}>
  <Card title="The chat input" icon="square-pen" href="/chat/composer">
    Attach files, pick the model, pause anytime.
  </Card>
