...
This is the easiest way to "pause and redirect" — type your new instruction and press Enter.

## What happens after a refresh

`Paused` is a frontend-only state. If you reload the page or close the tab, the colleague's status reverts to `Idle`. The Resume button disappears, but you can pick up the conversation by sending a new message — Eluu auto-resumes behind the scenes.
